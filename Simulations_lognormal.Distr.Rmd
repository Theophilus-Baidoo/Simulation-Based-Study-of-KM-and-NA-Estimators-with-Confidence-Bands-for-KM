...
```{r}
meanlog <- 0  # Mean parameter for log-normal distribution
sdlog <- 1    # Standard deviation for log-normal distribution
...
```{r}
# Define the log-normal CDF and its inverse
F <- function(x) plnorm(x, meanlog = meanlog, sdlog = sdlog)
Finv <- function(p) qlnorm(p, meanlog = meanlog, sdlog = sdlog)


doc <- read_docx()


pdf("LogNormal_KM_NA_Plots.pdf", width = 8, height = 6)
...
for (n in n_values) {
  for (censoring_rate in censoring_rates) {
    ...
    bias_plot <- ggplot(plot_data_bias, aes(x = Quantile, y = Bias, color = Estimator)) +
      ...
      geom_point(size = 2) +
      scale_color_manual(values = c("Kaplan-Meier" = "red", "Nelson-Aalen" = "blue")) +
      ggtitle(paste("Bias of Kaplan-Meier and Nelson-Aalen Estimators at Quantiles\n(Censoring Rate =", censoring_rate, ", Sample Size =", n, ")")) +
      xlab("Quantile") +
      ylab("Bias") +
      theme_minimal() +
      theme(plot.title = element_text(hjust = 0.5, size = 16, face = "bold")) 
    #+
     # annotate("text", x = 0.9, y = max(bias_km, bias_na) * 1.05, label = custom_label, hjust = 1, vjust = 1, size = 4, color = "black")
...
print(doc, target = "LogNormal_KM_NA_Plots.docx")
```
